...
class DataFetcher:
    ...
    def main(self):
        ...
        if sp500_data_points_count < 100:
            sp500_data_points, sp500_start, sp500_end = self.store_and_update(
                self.sp500_symbol, "stock", "sp500")
        else:
            sp500_data_points, sp500_start, sp500_end = sp500_data_points_count, None, None

        # fetch 25 bitcoin datapoints if we already have 100 sp500 data points and we do not already have 100 bitcoin datapoints, else fetch none
        if bitcoin_data_points_count < 100 <= sp500_data_points_count:
            bitcoin_data_points, bitcoin_start, bitcoin_end = self.store_and_update(
                "BTC", "crypto", "bitcoin")
        else:
    ...
    def create_table(self, table_name, data_type):
        ...
        cur = conn.cursor()
        cur.execute(
            f"CREATE TABLE IF NOT EXISTS {table_name} ({columns[data_type]})")
        conn.commit()
        conn.close()
    ...
    def get_bitcoin_weekly_data(self, start_date):
        ...
        # fetch all Mondays in this year (2020 or 2021)
        weekly_df = dataframe[dataframe['date'].dt.weekday == 0]
    ...
    def fetch_and_clean_data_with_retry(self, symbol, data_type, start_date, end_date):
        ...
        while retries < max_retries:
            try:
                data = self.fetch_and_clean_data(
                    symbol, data_type, start_date, end_date)
                return data
            except ValueError as err:
                print(
                    f"Error fetching data: {err}. Retrying... ({retries + 1}/{max_retries})")
                retries += 1
    ...
    def store_and_update(self, symbol, data_type, table_name):
        ...
        data = self.fetch_and_clean_data_with_retry(
            symbol, data_type, start_date, end_date)
